<template>
  <div class="keypad-wrap" ref="keypadBox">
    <div class="keypad-input">
      ...
      <span :class="`subVal ${insertIndex == index? 'active': ''}`" v-for="(item, index) in numbers" :key="index" @click="insert(index)">{{item}}</span>
    ...
    <transition name="slide-fade" mode="out-in">
      <div class="keypad-bar" ref="keypad" v-if="showKeypadBar">
        <div class="keypad-main">
          ...
          <div class="keypad-list">
            <key v-show="showDecimal" value="." @click="keyIn"></key>
            <key value="0" @click="keyIn" :style="showDecimal? '' : 'flex:2' "></key>
            <key value="hidden" :hidden="true" @click="keyIn"></key>
          </div>
        </div>
        <div class="keypad-handle">
          <key value="remove" :remove="true" @click="keyIn"></key>
          <key :value="confirmButtonText" @click="keyIn" style="background: rgba(251, 156, 30, 1); color: #ffffff; padding: 0 .3rem;"></key>
        </div>
      </div>
    </transition>
  </div>
  ...
  methods: {
    ...
    keyIn (value) {
      if (value == '.') { // 小数点
        ...
        if (integer.length > this.max) {  // 最大值判断 默认最大99亿
          this.numbers.splice(this.insertIndex, 1)
          this.insertIndex--
